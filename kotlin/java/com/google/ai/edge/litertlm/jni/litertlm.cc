...
LITERTLM_JNIEXPORT jlong JNICALL JNI_METHOD(nativeCreateSession)(
    ...
    jobject enable_speculative_decoding_obj) {
  auto session_config = SessionConfig::CreateDefault();

  if (auto enable_speculative_decoding =
          GetOptionalBoolean(env, enable_speculative_decoding_obj);
      enable_speculative_decoding.has_value()) {
    session_config.SetEnableSpeculativeDecoding(*enable_speculative_decoding);
  }
...
LITERTLM_JNIEXPORT jlong JNICALL JNI_METHOD(nativeCreateConversation)(
    ...
    jboolean enable_response_format, jobject enable_speculative_decoding_obj) {
  Engine* engine = reinterpret_cast<Engine*>(engine_pointer);
